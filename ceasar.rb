# This program will take a string and return a ceasar cipher 
# encoded string.

def ceasar_string(string, shift)

  alphabet = ('a'..'z').to_a # Make a Alphabet array
  ceasar_alphabet = alphabet.rotate(shift) # Rotate Alphabet for cipher
  ...
  string.each_char do |char| # Cycle through each character in string
    if alphabet.include?(char.downcase) # Check if char is alphabet or non-alphabet
      if char.ord.between?("A".ord, "Z".ord) # Check capitilization
        new_string.concat(ceasar_alphabet[alphabet.index(char.downcase)].upcase) # Update new string with upcase cipher
      else
        new_string.concat(ceasar_alphabet[alphabet.index(char.downcase)]) # Update new string with lowercase cipher
      end
    else
